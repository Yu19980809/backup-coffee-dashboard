...
import { fetchCoupons, setCouponOn, setCouponOff, offCoupons, addCoupon, editCoupon, deleteCoupon } from '../../store/actions'

// 添加/编辑 优惠券表单
const Form = ( { actionType, data, handleFormSubmit, handleCancel, selectedStartDate, setSelectedStartDate, selectedEndDate, setSelectedEndDate } ) => {
  const validationSchema = yup.object().shape({
    name: yup.string().required( 'required' ),
    value: yup.string().required( 'required' ),
  })

  const initialValues = actionType === 'add' ?  {
    name: '',
    value: '',
  } : {
    name: data.name,
  ...
  useEffect( () => {
    ...
    setSelectedStartDate( dayjs( data.start_date ) )
    setSelectedEndDate( dayjs( data.end_date ) )
...
const CouponManagement = () => {
  ...
  const [ selectedEndDate, setSelectedEndDate ] = useState( dayjs( new Date().getTime() + 3 * 24 * 60 * 60 * 1000 ) )
  ...
  // 请求优惠券列表数据
  useEffect( () => {
    dispatch( fetchCoupons() ).then( res => setCoupons( res ) )
  }, [ dispatch ] )

  // 切换生效状态
  const handleStatusSwitch = ( id, status ) => {
    if ( status === 'on' ) {
      dispatch( setCouponOff( id ) )
        .then( () => {
  ...
  const handleOffCoupons = () => {
    dispatch( offCoupons( { idList: selectedRows } ) )
      .then( () => {
  ...
  const handleAddCoupon = e => {
    const { name, value } = e
    dispatch( addCoupon( { name, value, start_date: selectedStartDate, end_date: selectedEndDate } ) )
      .then( res => {
  ...
  const handleEditCoupon = e => {
    const { name, value } = e
    dispatch( editCoupon( { name, value, start_date: selectedStartDate, end_date: selectedEndDate, id: clickedRow._id } ) )
      .then( res => {
        const newCoupons = coupons.map( item => item._id === res._id ? res : item )
        setCoupons( newCoupons )
  ...
  const handleDeleteCoupon = () => {
    dispatch( deleteCoupon( clickedRow._id ) )
      .then( () => {
  ...
  const columns = [
    ...
    { field: 'value', headerName: '面额', flex: 0.5 },
    ...
    },
    {
      ...
      renderCell: () => (
        <Box
          ...
          p='5px'
        >
          <CustomButton colors={ colors } text='编辑' handleClick={ () => { setEditCouponOpen( true ); setAddOrEdit( 'edit' ) } } />
  ...
  const CustomToolbar = () => (
    ...
    >
      ...
      <Box display='flex' gap='10px'>
        <CustomButton colors={ colors } text='添加优惠券' handleClick={ () => { setAddCouponOpen( true ); setAddOrEdit( 'add' ) } } />
  ...
  return (
    <Box m='20px'>
      ...
      <Modal
        open={ addOrEdit === 'add' ? addCouponOpen : editCouponOpen }
        onClose={ () => addOrEdit === 'add' ? setAddCouponOpen( false ) : setEditCouponOpen( false ) }
        ...
        >
          ...
          <Box id='modal-modal-description'>
            <Form
              ...
              handleFormSubmit={ addOrEdit === 'add' ? handleAddCoupon : handleEditCoupon }
              handleCancel={ () => addOrEdit === 'add' ? setAddCouponOpen( false ) : setEditCouponOpen( false ) }
            />
          </Box>
        </Box>
      </Modal>
